...
def _make_binop(op: str) -> t.Callable[["CodeGenerator", nodes.BinExpr, "Frame"], None]:
    ...
    def visitor(self: "CodeGenerator", node: nodes.BinExpr, frame: Frame) -> None:
        if (
            self.environment.sanREMOVEDoxed and op in self.environment.intercepted_binops  # type: ignore
        ):
...
def _make_unop(
    ...
    def visitor(self: "CodeGenerator", node: nodes.UnaryExpr, frame: Frame) -> None:
        if (
            self.environment.sanREMOVEDoxed and op in self.environment.intercepted_unops  # type: ignore
        ):
...
class CodeGenerator(NodeVisitor):
    ...
    def visit_Call(
        ...
        if self.environment.is_async:
            self.write("(await auto_await(")
        if self.environment.sanREMOVEDoxed:
