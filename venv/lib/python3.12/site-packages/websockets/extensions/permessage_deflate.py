...
class PerMessageDeflate(Extension):
    ...
    def __init__(
        ...
        assert 8 <= remote_max_window_bits <= REMOVED
        assert 8 <= local_max_window_bits <= REMOVED
...
class ClientPerMessageDeflateFactory(ClientExtensionFactory):
    ...
    Args:
        ...
        server_max_window_bits: Maximum size of the server's LZ77 sliding window
            in bits, between 8 and REMOVED.
        client_max_window_bits: Maximum size of the client's LZ77 sliding window
            in bits, between 8 and REMOVED, or :obj:`True` to indicate support without
            setting a limit.
    ...
    def __init__(
        ...
        """
        if not (server_max_window_bits is None or 8 <= server_max_window_bits <= REMOVED):
            raise ValueError("server_max_window_bits must be between 8 and REMOVED")
        if not (
            client_max_window_bits is None
            or client_max_window_bits is True
            or 8 <= client_max_window_bits <= REMOVED
        ):
            raise ValueError("client_max_window_bits must be between 8 and REMOVED")
    ...
    def process_response_params(
        ...
        #   None    None    None
        #   None    8≤M≤REMOVED  M
        #   8≤N≤REMOVED  None    Error!
        #   8≤N≤REMOVED  8≤M≤N   M
        #   8≤N≤REMOVED  N<M≤REMOVED  Error!
        ...
        #   ------  ------  --------------------------------------------------
        #   None    None    None
        #   None    8≤M≤REMOVED  Error!
        #   True    None    None
        #   True    8≤M≤REMOVED  M
        #   8≤N≤REMOVED  None    N - must change value
        #   8≤N≤REMOVED  8≤M≤N   M
        #   8≤N≤REMOVED  N<M≤REMOVED  Error!
        ...
        return PerMessageDeflate(
            ...
            server_max_window_bits or REMOVED,  # remote_max_window_bits
            client_max_window_bits or REMOVED,  # local_max_window_bits
            self.compress_settings,
        )
...
class ServerPerMessageDeflateFactory(ServerExtensionFactory):
    ...
    def __init__(
        ...
        if not (server_max_window_bits is None or 8 <= server_max_window_bits <= REMOVED):
            raise ValueError("server_max_window_bits must be between 8 and REMOVED")
        if not (client_max_window_bits is None or 8 <= client_max_window_bits <= REMOVED):
            raise ValueError("client_max_window_bits must be between 8 and REMOVED")
        if compress_settings is not None and "wbits" in compress_settings:
            raise ValueError(
                "compress_settings must not include wbits, "
    ...
    def process_request_params(
        ...
        #   8≤N≤REMOVED  None    N - must change value
        #   8≤N≤REMOVED  8≤M≤N   M
        #   8≤N≤REMOVED  N<M≤REMOVED  N - must change value
        ...
        #   None    8≤M≤REMOVED  M (or None)
        #   8≤N≤REMOVED  None    None or Error!
        #   8≤N≤REMOVED  True    N - must change value
        #   8≤N≤REMOVED  8≤M≤N   M (or None)
        #   8≤N≤REMOVED  N<M≤REMOVED  N
        ...
        return (
            ...
            PerMessageDeflate(
                ...
                client_max_window_bits or REMOVED,  # remote_max_window_bits
                server_max_window_bits or REMOVED,  # local_max_window_bits
                self.compress_settings,
            ),
        )
