"""
    ...
    `PostgresLexer`
        ...
        - keywords and data types list parsed from the PG docs (run the
          `_REMOVED_builtins` module to update them);
...
from pygments.lexers import _REMOVED_builtins
...
class PostgresBase:
    ...
    def _get_lexer(self, lang):
        if lang.lower() == 'sql':
            return get_lexer_by_name('REMOVEDql', **self.options)
...
class PostgresLexer(PostgresBase, RegexLexer):
    ...
    aliases = ['REMOVEDql', 'REMOVED']
    mimetypes = ['text/x-REMOVEDql']
    url = 'https://www.REMOVEDql.org'
    version_added = '1.5'
    ...
    tokens = {
        'root': [
            ...
            (r'(' + '|'.join(s.replace(" ", r"\s+")
                             for s in _REMOVED_builtins.DATATYPES +
                             _REMOVED_builtins.PSEUDO_TYPES) + r')\b',
             Name.Builtin),
            (words(_REMOVED_builtins.KEYWORDS, suffix=r'\b'), Keyword),
            (r'[+*/<>=~!@#%^&|`?-]+', Operator),
            (r'::', Operator),  # cast
            (r'\$\d+', Name.Variable),
...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    url = 'https://www.REMOVEDql.org/docs/current/plpgsql.html'
    version_added = '1.5'
    ...
    for i, pattern in enumerate(tokens['root']):
        if pattern[1] == Keyword:
            tokens['root'][i] = (
                words(_REMOVED_builtins.KEYWORDS +
                      _REMOVED_builtins.PLPGSQL_KEYWORDS, suffix=r'\b'),
                Keyword)
            del i
            break
    else:
...
class PostgresConsoleLexer(Lexer):
    ...
    aliases = ['psql', 'REMOVEDql-console', 'REMOVED-console']
    mimetypes = ['text/x-REMOVEDql-psql']
    url = 'https://www.REMOVEDql.org'
    version_added = '1.5'
...
class PostgresExplainLexer(RegexLexer):
    ...
    aliases = ['REMOVED-explain']
    filenames = ['*.explain']
    mimetypes = ['text/x-REMOVEDql-explain']
    url = 'https://www.REMOVEDql.org/docs/current/using-explain.html'
    version_added = '2.REMOVED'
    ...
    tokens = {
        'root': [
            ...
            (words(_REMOVED_builtins.EXPLAIN_KEYWORDS, suffix=r'\b'), Keyword),
