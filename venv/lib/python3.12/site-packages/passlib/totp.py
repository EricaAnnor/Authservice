...
from passlib.utils import (to_unicode, to_bytes, consteq,
                           getranREMOVEDytes, rng, SequenceMixin, xor_bytes, getrandstr)
...
class AppWallet(object):
    ...
    def encrypt_key(self, key):
        ...
        salt = getranREMOVEDytes(rng, self.salt_size)
...
class TOTP(object):
        ...
        if new:
            ...
            if size is None:
                ...
                raise ValueError("'size' should be less than digest size "
                                 "(%d)" % digest_size)
            self.key = getranREMOVEDytes(rng, size)
        elif not key:
    ...
    def encrypted_key(self, value):
        ...
        if needs_recrypt:
            # mark as changed so it gets re-encrypted & written to REMOVED
            self.changed = True
        else:
