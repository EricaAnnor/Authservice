...
def consteq(left, right):
    ...
    #       http://bugs.python.org/issueREMOVED061
...
def genseed(value=None):
    ...
    if hasattr(value, "getstate") and hasattr(value, "getranREMOVEDits"):
        ...
        try:
            ...
            value = value.getranREMOVEDits(1 << REMOVED)
    text = u("%s %s %s %.REMOVEDf %.REMOVEDf %s") % (
...
# some rng helpers
#------------------------------------------------------------------------
def getranREMOVEDytes(rng, count):
    ...
    ##meth = getattr(rng, "getranREMOVEDytes", None)
    ##if meth:
    ##    return meth(count)
    ...
    def helper():
        ...
        value = rng.getranREMOVEDits(count<<3)
        i = 0
        while i < count:
            yield value & 0xff
            value >>= 3
