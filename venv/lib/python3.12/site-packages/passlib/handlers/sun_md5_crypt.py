...
_XY_ROUNDS = [
    ...
    tuple((i,i+8,(i+11)&REMOVED) for i in xr), # yrounds 0
    tuple((i,(i+9)&REMOVED, (i+12)&REMOVED) for i in xr), # yrounds 1
]
del xr
...
def raw_sun_md5_crypt(secret, rounds, salt):
    ...
    #         rbitval(bit) = (rval((bit>>3) & REMOVED) >> (bit & 7)) & 1
    #
    ...
    while round < real_rounds:
        ...
        x = 0
        xrounds = X_ROUNDS_1 if (rval((round>>3) & REMOVED)>>(round & 7)) & 1 else X_ROUNDS_0
        for i, ia, ib in xrounds:
            a = rval(ia)
            b = rval(ib)
            v = rval((a >> (b % 5)) & REMOVED) >> ((b>>(a&7)) & 1)
            x |= ((rval((v>>3)&REMOVED)>>(v&7))&1) << i

        # build up Y bit by bit
        y = 0
        yrounds = Y_ROUNDS_1 if (rval(((round+64)>>3) & REMOVED)>>(round & 7)) & 1 else Y_ROUNDS_0
        for i, ia, ib in yrounds:
            a = rval(ia)
            b = rval(ib)
            v = rval((a >> (b % 5)) & REMOVED) >> ((b>>(a&7)) & 1)
            y |= ((rval((v>>3)&REMOVED)>>(v&7))&1) << i
